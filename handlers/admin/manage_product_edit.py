...
from aiogram.types import CallbackQuery, Message, ContentType as CT
...
from utils.account_sorter import delete_product_folder, handle_product_file
...
@router.callback_query(ManageProductEdit.filter(F.sub_action == 'add_quantity'))
async def handle(call: CallbackQuery, state: FSMContext, callback_data: ManageProductEdit):
    keyboard = get_manage_product_edit_keyboard(id=callback_data.id)
    text = await get_product_text_edit(state=state,
                                       end_text=f'"Отправьте текстовый документ или zip-aрхив"\n\n'
                                                'Проверьте соответствие формату заполнения:\n'
                                                '\t ❕ Для текстовых - строки с разеделителем (указать разделитель!)\n'
                                                '\t ❕ Для zip-архивов - zip-архив с расширением .zip и каждый аккаунт в своей папке')
    msg = await call.message.answer(text=text, reply_markup=keyboard)
    data = await state.get_data()
    
    await clear_chat(data=data)
    data['items_to_del'].append(msg)
    data['product_id_add_quantity'] = callback_data.id
    await state.update_data(data)
    await state.set_state(ProductEditState.edit_quantity)
...
async def handle(message: Message, state: FSMContext):
    ...
    product_id = data['product_id_add_quantity']
